...
(() => {
  ...
  roomsData.forEach(room => {
    if (!_grouped[room.category]) _grouped[room.category] = [];
    _grouped[room.category].push(room);
  });
  ...
  window.__cyberData = Object.keys(_meta)
    ...
    return {
      ...
      icon: m[0],
      theme: m[1],
      links: _grouped[c].map(room => ({
        name: room.name,
        url: 'https://tryhackme.com/room/' + (room.slug || _urlName(room.name)),
        desc: room.description,
        tags: [room.difficulty.toLowerCase(), ...room.tags],
